  ...
  return (
    <>
      <div className="container bg-[#F7ECE4] overscroll-y-scroll h-screen">
        ...
        <div className="text-[#6B003A] font-bold text-[18px] ml-5 pt-2">Picture</div>
        <div onClick={handleUploadPhoto} className={'ml-5 mr-5 bg-[#FFDFC2] h-[212px] rounded-2xl flex flex-row justify-center items-center'}>
          {
            avatarUrl ?
              ...
              <div className="w-[155px] h-[48px] border-solid border-2 border-[#6B003A] rounded-[24px] flex flex-row justify-center items-center">
                <span className="text-[#6B003A] font_medium_bold">Upload Photo</span>
              </div>
          }
        </div>
        {avatarUrl && <div className="ml-5 mr-5 mt-2 flex flex-row flex-wrap">
          {photoType.map((item, index: number) => (
            <span onClick={() => handleSelect(index)} className={`block p-2 mr-2 mb-2 text-[14px] text-[#6B003A] font-bold rounded-2xl ${index === selectedIndex.current ? 'bg-[#FBB1A1]' : 'bg-[#FFFFFF]'}`} key={item}>{item}</span>
          ))}
        </div>
        }
